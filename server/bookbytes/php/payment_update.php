...
$signed= hash_hmac('sha256', $signing, 'S-YE8TAEa6ByRUKxczk9m6gQ');
if ($signed === $data['x_signature']) {
    if ($paidstatus == "Success"){ //payment success
     
        $sqlcart = "SELECT * FROM `tbl_carts` WHERE `cart_status`='New' AND `user_id_id` = '$userid' ORDER BY `user_id`;";
            ...
            while ($row = $result->fetch_assoc()) {
               ...
               // print_r($cartarray);
                ...
                if ($rows == 1){
                    ...
                    $total = $price * $qty;
                    $sqlinsertorder = "INSERT INTO `tbl_orders`(`user_id`, `seller_id`, `order_total`, `order_status`) VALUES ('$userid','$seller','$total','$status')";
